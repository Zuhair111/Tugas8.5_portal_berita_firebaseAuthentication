            ...
            <div class="flex items-center justify-between py-4">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    @else
                        ...
                        <a href="{{ route('register') }}" class="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700">
                            Daftar
                        </a>
                    @endauth
                    <button class="p-2">
    ...
    <nav class="bg-white border-b sticky top-16 z-40">
        <div class="container mx-auto px-4">
            <div class="flex space-x-8 overflow-x-auto py-3">
                <a href="/" class="text-red-600 font-semibold whitespace-nowrap border-b-2 border-red-600 pb-2">Berita Utama</a>
                @foreach($categories->take(6) as $category)
                <a href="/kategori/{{ $category->slug }}" class="text-gray-700 hover:text-red-600 whitespace-nowrap pb-2">{{ $category->name }}</a>
                @endforeach
            </div>
        </div>
    </nav>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                <div>
                    <h3 class="text-xl font-bold mb-4">
                        <span class="text-red-500">PORTAL</span><span class="bg-red-500 px-2 py-1 rounded">BERITA</span>
                    </h3>
                    <p class="text-gray-400 text-sm">Portal berita terpercaya dan terkini di Indonesia</p>
                </div>
                <div>
                    <h4 class="font-semibold mb-4">Kategori</h4>
                    <ul class="space-y-2 text-sm text-gray-400">
                        @foreach($categories->take(5) as $category)
                        <li><a href="/kategori/{{ $category->slug }}" class="hover:text-white">{{ $category->name }}</a></li>
                        @endforeach
                    </ul>
                </div>
                <div>
